...
func (app *application) registerUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = app.models.Permissions.AddForUser(int64(user.ID), "articles:read")
	if err != nil {
		fmt.Printf("here3")
		app.serverErrorResponse(w, r, err)
		return
	...
	// token for the user.
	token, err := app.models.Tokens.New(int64(user.ID), 3*24*time.Hour, model.ScopeActivation)
	if err != nil {
		fmt.Printf("here4")
		app.serverErrorResponse(w, r, err)
		return
...
func (app *application) activateUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Retrieve the details of the user associated with the Tokens using the GetForToken() method.
	...
	err = app.models.Tokens.DeleteAllForUser(model.ScopeActivation, int64(user.ID))
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
