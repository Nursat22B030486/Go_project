...
func (app *application) createAuthenticationTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	token, err := app.models.Tokens.New(int64(user.ID), 24*time.Hour, model.ScopeAuthentication)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
	...
	// status code.
	err = app.writeJSON(w, http.StatusCreated, envelope{"authentication_token": token, "user_id": int64(user.ID)}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
